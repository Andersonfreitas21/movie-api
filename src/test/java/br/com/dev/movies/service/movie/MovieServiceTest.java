...
class MovieServiceTest {
    ...
    @Test
    void findAllMovies() throws JsonProcessingException {
        ObjectMapper mapper = new ObjectMapper();

        TypeFactory typeFactory = mapper.getTypeFactory();
        List<Movie> movieArrayList = mapper.readValue(MOVIES_LIST, typeFactory.constructCollectionType(List.class, Movie.class));

        when(movieRepository.findAll()).thenReturn(movieArrayList);
    ...
    @Test
    void findMovieByImdbId() throws JsonProcessingException {
        ObjectMapper mapper = new ObjectMapper();
        Movie movieExpected = mapper.readValue(MOVIE, Movie.class);
        String imdbId = "tt1234567";

        when(movieRepository.findMovieByImdbId(imdbId)).thenReturn(Optional.ofNullable(movieExpected));

        Optional<Movie> movieActual = movieService.findMovieByImdbId(imdbId);
        assertTrue(movieActual.isPresent());
        assertEquals(movieExpected, movieActual.get());
    }
    ...
    private static final String MOVIE = """
            ...
            """;
    private static final String MOVIES_LIST = """
            [
              {
                "imdbId": "tt3915174",
                "title": "Puss in Boots: The Last Wish",
